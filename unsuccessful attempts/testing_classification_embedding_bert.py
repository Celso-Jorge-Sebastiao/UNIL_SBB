# -*- coding: utf-8 -*-
"""TESTING Classification / Embedding BERT

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1jne2ubzRvUcD_ixdzaAxs56LLOg_wzYX
"""
...
#data['category'] = data['difficulty'].apply(lambda x: 'A' if x in ['A1', 'A2'] else "C")
data['category'] = data['difficulty'].apply(lambda x: 'A' if x in ['A1', 'A2'] else ('B' if x in ['B1', 'B2'] else 'C'))
data['word_count'] = data['sentence'].apply(lambda x: len(x.split()))
data['virgules'] = data['sentence'].apply(lambda x: x.count(','))
resultats_statistiques = data.groupby('category')['sentence'].apply(
    lambda x: x.str.split().apply(len).agg(['mean', 'min', 'max']))
print(resultats_statistiques)
resultats_statistiques = data.groupby('difficulty')['virgules'].agg(['mean', 'min', 'max'])

# Affichez les résultats statistiques
print(resultats_statistiques)
data.head()

X_train, X_test, y_train, y_test = train_test_split(data['word_count'].values.reshape(-1, 1), data['category'], test_size=0.2, random_state=42)
...
filtered_a = data[data['difficulty'].isin(['A1', 'A2'])]
filtered_b = data[data['difficulty'].isin(['B1', 'B2'])]
filtered_c = data[data['difficulty'].isin(['C1', 'C2'])]

X_train_a, X_test_a, y_train_a, y_test_a = train_test_split(filtered_a['embedding'].tolist(), filtered_a['difficulty'], test_size=0.2, random_state=42)
X_train_b, X_test_b, y_train_b, y_test_b = train_test_split(filtered_b['embedding'].tolist(), filtered_b['difficulty'], test_size=0.2, random_state=42)
X_train_c, X_test_c, y_train_c, y_test_c = train_test_split(filtered_c['embedding'].tolist(), filtered_c['difficulty'], test_size=0.2, random_state=42)

classifier_a = LogisticRegression(penalty='l2', solver='lbfgs', max_iter=1000)
classifier_a.fit(X_train_a, y_train_a)
y_pred_a = classifier_a.predict(X_test_a)
accuracy_a = accuracy_score(y_test, y_pred)
classification_report_str_a = classification_report(y_test_a, y_pred_a)

classifier_b = LogisticRegression(penalty='l2', solver='lbfgs', max_iter=1000)
classifier_b.fit(X_train_b, y_train_b)
y_pred_b = classifier_b.predict(X_test_b)
accuracy_b = accuracy_score(y_test_b, y_pred_b)
classification_report_str_b = classification_report(y_test_b, y_pred_b)

classifier_c = LogisticRegression(penalty='l2', solver='lbfgs', max_iter=1000)
classifier_c.fit(X_train_c, y_train_c)
y_pred_c = classifier_c.predict(X_test_c)
accuracy_c = accuracy_score(y_test_c, y_pred_c)
classification_report_str_c = classification_report(y_test_c, y_pred_c)
# Affichez les résultats
print("Classification Report:\n", classification_report_str_a)
print("Classification Report:\n", classification_report_str_b)
print("Classification Report:\n", classification_report_str_c)
...
print("Stacking - Accuracy: {:.2f}, Precision: {:.2f}, Recall: {:.2f}, F1 Score: {:.2f}".format(accuracy_stacking, precision_stacking, recall_stacking, f1_stacking))
print("LR - Accuracy: {:.2f}, Precision: {:.2f}, Recall: {:.2f}, F1 Score: {:.2f}".format(accuracy_LR, precision_LR, recall_LR, f1_LR))
print("KNN - Accuracy: {:.2f}, Precision: {:.2f}, Recall: {:.2f}, F1 Score: {:.2f}".format(accuracy_KNN, precision_KNN, recall_KNN, f1_KNN))
print("GaussianNB - Accuracy: {:.2f}, Precision: {:.2f}, Recall: {:.2f}, F1 Score: {:.2f}".format(accuracy_GaussianNB, precision_GaussianNB, recall_GaussianNB, f1_GaussianNB))
print("Kernel - Accuracy: {:.2f}, Precision: {:.2f}, Recall: {:.2f}, F1 Score: {:.2f}".format(accuracy_kernel, precision_kernel, recall_kernel, f1_kernel))
print("RF - Accuracy: {:.2f}, Precision: {:.2f}, Recall: {:.2f}, F1 Score: {:.2f}".format(accuracy_RF, precision_RF, recall_RF, f1_RF))
print("Voting - Accuracy: {:.2f}, Precision: {:.2f}, Recall: {:.2f}, F1 Score: {:.2f}".format(accuracy_voting, precision_voting, recall_voting, f1_voting))

from google.colab import files
data['embedding'].to_csv('data_embeded.csv', index=False)
files.download('data_embeded.csv')
